  ...
  module Adapters
    class EventSource < Adapter
      ...
      def handle(request)
        ...
        body = "\r\n\r\n"
        self.write_response(200, {"Content-Type" => CONTENT_TYPES[:event_stream], "Cache-Control" => "no-store, no-cache, must-revalidate, max-age=0", "Set-Cookie" => "JSESSIONID=#{request.session_id}; path=/"}, body)
      end
